...
def Model(X, Y, test_x, test_y, batch):
    ...
    model.fit(X, Y, epochs=1, batch_size=batch, validation_data=(test_x, test_y))
    pred = model.predict(test_x)
    return pred

def Model_CNN(train_data, train_target, test_data, test_target, batch):
    IMG_SIZE = 32
    ...
    pred = Model(Train_X, train_target, Test_X, test_target, batch)
    pred[pred >= 0.5] = 1
    pred[pred < 0.5] = 0
